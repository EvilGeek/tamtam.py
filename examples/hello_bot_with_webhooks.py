...
async def server_setup(url):
    info = await bot.subscribe(types.NewSubscriptionConfig(url=str(url)))
...
if __name__ == "__main__":
    ...
    my_url = yarl.URL.build(scheme=scheme, host=host, port=port).with_path(path)

    run_async(server_setup(my_url))
    # till here

    run_server(port=port, host=host, path=path)
